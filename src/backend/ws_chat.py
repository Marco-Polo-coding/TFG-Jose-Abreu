...
async def websocket_chat(websocket: WebSocket, chat_id: str, uid: str = Depends(get_current_uid_ws)):
    # La conexión ya fue aceptada en get_current_uid_ws
    ...
    try:
        while True:
            ...
            event = msg.get("event")
            if event == "message":
                # Guardar mensaje usando la lógica REST
                ...
                # Notificar a todos los clientes del chat
                await broadcast(chat_id, {"event": "message", "message": message_obj})
            elif event == "typing":
                typing_users[chat_id].add(uid)
                # Enviar el evento de typing a todos excepto al usuario que está escribiendo
                ...
                # Enviar el evento de stop_typing a todos excepto al usuario que dejó de escribir
                await broadcast(chat_id, {"event": "typing", "user": uid, "typing": False}, exclude_uid=uid)
            elif event == "read":
                # Marcar como leído usando la lógica REST
